...
func HashMatch(ctx context.Context, head *filesync.HashHead, srcFile *file.File) (matchIter iter.Seq2[*filesync.FileChunk, error], err error) {
	var (
		buf         []byte
		fileSize    int64
		offset      int64
	...
	yieldNotMatchData := func(force bool, yield func(*filesync.FileChunk, error) bool) {
		if force || len(notMatchData) >= int(head.GetBlockLength()) {
			yield(&filesync.FileChunk{Data: notMatchData}, nil)
			notMatchData = notMatchData[:0]
		}
	}

	matchIter = func(yield func(*filesync.FileChunk, error) bool) {
		fi, err := srcFile.Stat()
		if err != nil {
			yield(nil, err)
			return
		...
		// read the first filesync.FileChunk with offset: 0
		...
		for {
			...
			if match {
				...
				for ; blockIdx < head.GetCheckSumCount(); blockIdx++ {
					...
					hashHits++
					if !yield(&filesync.FileChunk{Hash: hashs[blockIdx]}, nil) {
						return
					}
